...
const MentorInput = ({
  ...
  const handleAssignToMentor = async () => {
    ...
    const submissionRef = doc(firestore, "TestEvalRequests", submissionData.id);
  ...
  const handleSubmit = async () => {
    ...
    if (!isDocumentValid) Alert.alert("Error", "Upload a valid PDF.");
    ...
    try {
      ...
      const fileRef = ref(
        storage,
        `test-eval-requests/${submissionData.id}/mentor/${evaluationFile.name}`
      );
